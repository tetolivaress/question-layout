<template lang="pug">
  v-container
    v-card.ma-12.pa-6
      ...
      v-row.mt-4
        v-col.d-flex.justify-center(cols='2') Producto
        v-col(cols='4')
          v-select(solo label="Producto" :items="products" v-model="form.product")
        v-col.d-flex.justify-center(cols='2') Modulo
        v-col(cols='4')
          v-select(
            v-model="form.module"
            solo
      ...
      v-row
        v-col.d-flex.justify-center(cols='2') Categorias
        v-col(cols='4')
          v-select(solo label="Categorias" :items="categories" v-model="form.category")
        v-col.d-flex.justify-center(cols='2') Entorno
        v-col(cols='4')
          v-select(
